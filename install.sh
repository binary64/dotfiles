...
echo "YES" | cryptsetup luksFormat /dev/disk/by-partlabel/${PART_ROOT} --key-size 512 --hash sha512 --key-file /tmp/keyfile
echo "Encryption setup."

echo "$passphrase" | cryptsetup luksAddKey /dev/disk/by-partlabel/${PART_ROOT} --key-file /tmp/keyfile
echo "added passphrase"

# mount the cryptdisk at /dev/mapper/nixroot
cryptsetup luksOpen /dev/disk/by-partlabel/${PART_ROOT} nixroot -d /tmp/keyfile
echo "Disk mounted."

# remove the temporary keyfile
cryptsetup luksRemoveKey /dev/disk/by-partlabel/${PART_ROOT} /tmp/keyfile
rm -f /tmp/keyfile
